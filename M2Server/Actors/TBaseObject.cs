    ...
    public class TBaseObject : GameBase
    {
            ...
            try
            {
                ...
                switch (ProcessMsg.wIdent)
                {
                    ...
                    case Grobal2.RM_MAGSTRUCK_MINE:
                        ...
                        {
                            nDamage = GetMagStruckDamage(HUtil32.IntToObject<TBaseObject>(ProcessMsg.BaseObject), ProcessMsg.nParam1);// 修正学了内功后对魔法技能不增加攻击力
                        }
                        else
                        {
        ...
        public bool EnterAnotherMap(TEnvirnoment Envir, int nDMapX, int nDMapY)
        {
            ...
            try
            {
                ...
                nCode = 4;
                MapCellInfo = new TMapCellinfo();
                if (!Envir.GetMapCellInfo(nDMapX, nDMapY, ref MapCellInfo))//取地图路径数组失败，则跳出
                {
                    return result;
                }
                nCode = 5;
                ...
                || !(InSafeZone() && BaseObject.InSafeZone()))
            {
                if (IsProperTarget(BaseObject))
                {
                    if (HUtil32.Random(BaseObject.m_btSpeedPoint) < m_btHitPoint)
                    {
                        ...
                        BaseObject.m_WAbil.HP, BaseObject.m_WAbil.MaxHP, this.ToInt(), "", 500);//增加英雄过滤
                                ...
                                switch (nCode)
                                {
                                    case 0:// 物品禁止扔
                                        if (CheckItem.boCanDrop)
                                        {
                                            if (M2Share.g_ManageNPC != null)
                                            {
                                                if (m_btRaceServer == Grobal2.RC_PLAYOBJECT)
                                                {
                                                    SendMsg(M2Share.g_ManageNPC, Grobal2.RM_MENU_OK, 0, this.ToInt(), 0, 0, "此物品禁止扔在地上！！！");
                                                }
                                                else if ((m_btRaceServer == Grobal2.RC_HEROOBJECT))
                                                {
                                                    ((THeroObject)(this)).SendMsg(M2Share.g_ManageNPC, Grobal2.RM_MENU_OK, 0, this.ToInt(), 0, 0, "此物品禁止扔在地上！！！");
                                                }
                                            }
                                            result = true;
                                            break;
                                    ...
                                    case 1:// 0 物品禁止交易  *
                                        if (CheckItem.boCanDeal)
                                        {
                                            ...
                                            {
                                                SendMsg(M2Share.g_ManageNPC, Grobal2.RM_MENU_OK, 0, this.ToInt(), 0, 0, "此物品禁止交易！！！");
                                            }
                                            result = true;
                                            break;
                                        }
                                    ...
                                    case 2:// 1 物品禁止存
                                        if (CheckItem.boCanStorage)
                                        {
                                            if (M2Share.g_ManageNPC != null)
                                            {
                                                SendMsg(M2Share.g_ManageNPC, Grobal2.RM_MENU_OK, 0, this.ToInt(), 0, 0, "此物品禁止存仓库！！！");
                                            }
                                            result = true;
                                            break;
                                        }
