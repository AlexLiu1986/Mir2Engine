                    ...
                    if (this.IsProperTarget(BaseObject))
                    {
                        if ((Math.Abs(this.m_nCurrX - BaseObject.m_nCurrX) <= 1) && (Math.Abs(this.m_nCurrY - BaseObject.m_nCurrY) <= 1))
                        {
                            ...
                            if (n10 > 0)
                            {
                                ...
                                BaseObject.SendDelayMsg(Grobal2.RM_STRUCK, Grobal2.RM_10101, n10, BaseObject.m_WAbil.HP, BaseObject.m_WAbil.MaxHP, this.ToInt(), "", 700);
                            }
                        }
                    }
                }
        ...
        private void sub_4A6B30(TBaseObject TargeTBaseObject)
        {
            ...
            if (TargeTBaseObject != null)
            {
                ...
                if (nPower > 0)
                {
                    ...
                    TargeTBaseObject.SendDelayMsg(Grobal2.RM_STRUCK, Grobal2.RM_10101, nPower, TargeTBaseObject.m_WAbil.HP, TargeTBaseObject.m_WAbil.MaxHP, this.ToInt(), "", 
                ...
                this.SendRefMsg(Grobal2.RM_FLYAXE, this.m_btDirection, this.m_nCurrX, this.m_nCurrY, TargeTBaseObject.ToInt(), "");
            }
        }
    ...
    public class TArcherGuardMon1 : TAnimalObject
    {
        ...
        public override void Run()
        {
            try
            {
                if (!this.m_boDeath && !this.m_boGhost && (this.m_wStatusTimeArr[Grobal2.POISON_STONE] == 0))
                {
                    if ((((int)HUtil32.GetTickCount() - this.m_dwWalkTick) > this.m_nWalkSpeed) && m_boWalk)// 走动
                    {
                        this.m_dwWalkTick = HUtil32.GetTickCount();
