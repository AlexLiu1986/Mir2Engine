                ...
                if (!PlayObject.m_PEnvir.m_boNOPOSITIONMOVE)
                {
                    if (PlayObject.m_PEnvir.CanWalkOfItem(nX, nY, M2Share.g_Config.boUserMoveCanDupObj, M2Share.g_Config.boUserMoveCanOnItem))
                    {
                        if ((HUtil32.GetTickCount() - PlayObject.m_dwTeleportTick) > M2Share.g_Config.dwUserMoveTime * 1000)
                        {
                            ...
                            if ((PlayObject.m_UseItems[TPosition.U_BUJUK]->wIndex > 0) && (PlayObject.m_UseItems[TPosition.U_BUJUK]->Dura > 0))// 增加传送符功能
                            {
                                if (PlayObject.m_UseItems[TPosition.U_BUJUK]->Dura > 100)
                                {
                                ...
                                PlayObject.SendMsg(PlayObject, Grobal2.RM_DURACHANGE, TPosition.U_BUJUK, PlayObject.m_UseItems[TPosition.U_BUJUK]->Dura,
                                    PlayObject.m_UseItems[TPosition.U_BUJUK]->DuraMax, 0, "");
